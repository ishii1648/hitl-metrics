...
| `~/.claude/logs/permission.log` | 1イベント1行 | timestamp, session_id, enriched tool_name（`Bash(git)`, `Edit(internal/syncdb)`, `Read(docs/adr)` 形式） |
...
| tool | TEXT | enriched ツール名（`Bash(git)`, `Edit(internal/syncdb)`, `Read(docs/adr)` 等） |
...
| AskUserQuestion 週別トレンド | Time series | 仕様不確実性の推移 |
| PR 別 perm_rate ランキング | Bar chart | PR ごとの perm_rate ソート |
| PR 別統計テーブル | Table | pr_url, task_type, session_count, tool_use_total, mid_session_msgs, ask_user_question, perm_count, perm_rate, review_comments, changes_requested |
