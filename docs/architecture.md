...
3. pr_metrics VIEW がスキーマ定義済みのため自動利用可能
```
...
| PR 別 token スコアカード | Table | pr_url, task_type, model, total_tokens, tokens_per_session, tokens_per_tool_use, pr_per_million_tokens, token 内訳, session_count, peak_parallel_sessions, tool_use_total, mid_session_msgs, ask_user_question, review_comments, changes_requested |
