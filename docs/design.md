...
ローカル `~/.claude/agent-telemetry.db` に閉じていたメトリクスを、複数マシン・複数ユーザのデータを統合できるサーバへ送る経路を追加する。クライアントは push 型で能動的に送り、サーバはクライアントと同一の SQLite スキーマでデータを蓄積する。ローカル Grafana とサーバ Grafana が同じダッシュボード JSON を共有できるよう、`internal/syncdb/` の集計ロジックをサーバ binary で再利用する。

### 送信ペイロード — raw JSONL 転送

サーバには `session-index.jsonl` の差分行と、対応する transcript JSONL を **そのまま** 送る。集計済み値（`transcript_stats`）だけを送る案や、OTLP / Prometheus remote write への変換は採用しない。

理由:

- 「ソース・オブ・レコードは `session-index.jsonl` と transcript JSONL であり、SQLite はあくまで集計キャッシュ」という方針（前述）と整合する。SoR をそのまま送るのが一貫性を保つ唯一の方法
- サーバ側でも同じ `internal/syncdb/` ロジックを走らせれば、ローカル DB とサーバ DB が決定的に一致する
- 将来 transcript から追加の指標を抽出したくなった場合、サーバ側で再集計できる（集計値だけ送る案では不可能）
- transcript の中身は既に Anthropic API に context として送信済みであり、自前 telemetry サーバへ送ることに追加のプライバシー懸念は本質的にない。保存期間・閲覧範囲はサーバ運用側のポリシーで吸収する

OTLP / Prometheus を採用しない理由:

- agent-telemetry のデータは「PR 単位の構造化レコード」であり、時系列のスクレイプではない。`is_merged` / `pr_url` / `review_comments` は backfill が後追いで書き換えるため、append-only に近い OTLP 経路と相性が悪い
- ローカル SQLite + Grafana スタックと OTel collector / Prometheus / Mimir スタックが二重に存在することになり、検証経路が増える割に得るメタデータが薄い
- `docs/metrics.md` の OpenMetrics カタログは観察軸の整理として価値があるが、収集経路の固定化を意味しない。将来 `/metrics` エンドポイントを生やす際の export 形式仕様としてのみ有効
...
- HTTP gzip は **必須**（JSONL は冗長で 1/5〜1/10 に縮む）
- Codex の `.jsonl.zst` は再展開せずそのまま `encoding: "zstd"` で詰めて送る。サーバは受信時に encoding を見て展開要否を判定する
- 1 リクエストあたり最大 50 MB を上限とし、超える場合はクライアントが複数リクエストに分割する
...
```json
{
  ...
  "pushed_session_versions": {
    "<session_id>": "<sha256 of session-index.jsonl line + transcript content>"
  }
}
```

- 各 push 時に session-index.jsonl の各行と transcript の hash を取り、`pushed_session_versions` と比較
- hash 一致 → 既送信、スキップ
- hash 不一致 → 後追い更新あり、再送信
- transcript は session 完了後（`ended_at` 非空かつ `end_reason` 設定済み）のみ対象。進行中セッションは除外
- `agent-telemetry push --full` は `pushed_session_versions` を無視してフルスキャンする

`last_backfill_offset` 単独では backfill の後追い更新（`is_merged` / `pr_url` / `review_comments` 等）を取りこぼすため、行単位の hash で差分検知する。
...
`user_id`（人物識別）は payload に含まれる。**API key の認証**（信頼境界）と **`user_id` 経路**（集計軸）は責務を分ける。チーム内信頼を前提とし、人物単位の write 制限や OIDC は需要が出てから追加する。

### サーバ側 — ingest API + 共通 syncdb
...
internal/
  serverpipe/                        # ingest ハンドラ・差分判定・syncdb 起動
```
...
<server_data_dir>/
  <user_id>/
    session-index.jsonl              # 受信行を append
    transcripts/<session_id>.jsonl.zst
  agent-telemetry.db                 # 全 user 集約 SQLite（クライアントと同一スキーマ）
...
1. Bearer token を検証
2. 受信 payload を `user_id` ごとに分配し、`session-index.jsonl` に append、transcript を `transcripts/` 配下に **zstd 保管**
3. 受信完了後、内部で `internal/syncdb/` を呼び出して SQLite を upsert（クライアントと同一実装）
4. レスポンスとして受信件数 / スキップ件数 / 衝突件数を返す

クライアントとサーバで `internal/syncdb/` を共有するため、SoR が同じなら集計結果は決定的に一致する。
...
| ケース | 圧縮前 | 圧縮後（gzip 概算） |
|---|---|---|
| 個人 1 日（10〜30 セッション × 平均 1 MB） | 10〜30 MB | 2〜6 MB |
| 個人 1 ヶ月 | 300〜900 MB | 60〜180 MB |
| 5 人チーム 1 ヶ月 | 1.5〜4.5 GB | 300〜900 MB |

ピーク（長セッションで 50 MB クラス）はバッチサイズ上限 50 MB を超える可能性があり、その場合は 1 セッション 1 リクエストに分割する。

サーバストレージが圧迫されたら `agent-telemetry server gc --transcript-before <date>` で古い transcript を削除する（集計値は SQLite に残り、過去指標の参照は維持される）。GC は MVP には含めず、必要になった時点で追加する。
...
- transcript 全文をサーバに保管するためサーバストレージが線形に増加する。GC コマンドは MVP には含まれず、増えたら手動削除または運用判断で対応する
