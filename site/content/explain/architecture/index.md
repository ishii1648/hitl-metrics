...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 50, 'rankSpacing': 90}, 'themeVariables': {'fontSize': '20px'}}}%%
flowchart LR
    ...
    subgraph state["2. オンディスク (JSONL)"]
        direction TB
        CCJ["~/.claude/<br/>session-index<br/>+ transcript"]
        CXJ["~/.codex/<br/>session-index<br/>+ rollout"]
    end
    ...
    DB[("agent-telemetry.db<br/>(SQLite)")]
