...
merged PR 数、平均 perm rate、PR 平均 mid-session メッセージ数、レビューコメント数、changes requested 数の指標。**perm rate が低いほど Claude が自律的に作業できています**。赤は 15% 以上（改善が必要）、緑は 5% 未満（良好）。
...
ツール別の permission 内訳を表示。**上位ツールを allowlist に追加すれば perm rate を直接改善**できます。例えば `Bash(git)` が 40% を占めていれば、git 系コマンドを許可するだけで perm rate が 4 割改善します。
