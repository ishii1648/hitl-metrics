...
代わりに **dev tool として独立 Go プログラム** を `tools/intent/` に置く：

```
tools/intent/main.go         # 独立 Go プログラム
.claude/skills/intent-lookup/ # Claude Code から呼ぶ skill
Makefile                     # `make intent PATH=...` target を追加
```

- `tools/intent/` は `cmd/agent-telemetry/` と分離。**goreleaser の build target に追加しない**（`.goreleaser.yaml` を変更しない）
...
- `--format=json` で機械可読出力（Claude が context として読み込みやすくする / 将来 0011 段階 4 で Hugo build に再利用する場合の中間データとしても流用可）
...
- [ ] `tools/intent/` に独立 Go プログラムを実装（frontmatter 走査 + `git log --follow` の Contextual Commits 行抽出をマージ）
- [ ] **`cmd/agent-telemetry/` には変更を加えず、`.goreleaser.yaml` の build target にも追加しない**
- [ ] `--format=json` / 既定 markdown 出力の両方をサポート
- [ ] `Makefile` に `make intent PATH=<p>` target を追加
- [ ] `.claude/skills/intent-lookup/` を新設（Claude Code 経由で自然に呼べるようにする）
- [ ] tools/intent のテスト（fixture issue + git log で逆引きが期待通り返ることを検証）
