...
### スコープ外（フォロー issue で扱う）

- `~/.claude/agent-telemetry.db`（DB 本体）の移行: Grafana mount path・`backfill`・`sync-db` 全体に影響するため別 issue
- `~/.claude/agent-telemetry-state.json` / `~/.codex/agent-telemetry-state.json`（push 済み session 追跡）の移行: 同上
- session-index.jsonl は agent 固有のデータ（`~/.claude/session-index.jsonl` / `~/.codex/session-index.jsonl`）なので agent ディレクトリにあるのが筋であり、移行対象ではない

### 想定される波及

- dotfiles で `~/.claude/agent-telemetry.toml` を配布している既存ユーザは、`~/.config/agent-telemetry/config.toml` への symlink もしくは移動が必要。doctor / setup の案内文と docs/setup.md の手順で吸収する
- CI 等で `~/.claude/agent-telemetry.toml` を直書きしている場合は同様に更新が必要だが、本リポジトリ内の test fixture は `t.TempDir()` ベースなので影響なし
