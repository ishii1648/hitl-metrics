...
description: Use when the user says "impl", "実装", "実装セッション起動",
  ...
  Parses issues/ and launches worktree + tmux session + Claude Code sessions.
version: 0.2.0
---

# impl

`issues/` 直下の open issue（`issues/<NNNN>-<cat>-<slug>.md`）を検出し、issue ごとに worktree + tmux session + Claude Code を起動して並列実装をディスパッチする。`issues/closed/` と `issues/pending/` は対象外。
