...
`$PWD/TODO.md` を Read して「未着手」セクションからタスクを抽出する。
...
- `<session_name>`: worktree パスの basename（例: `claudedog@feat-adr-017`）

### Step 6: Claude Code 起動と初期プロンプト送信

各 tmux session に対して以下を実行する:
...
```bash
sleep 5
tmux send-keys -t "<session_name>" "<初期プロンプト>" Enter
```
...
dispatch 完了:
  ...
  1. claudedog@feat-adr-017 → 初期プロンプト送信済み
  2. claudedog@feat-bash-context-monitoring → 初期プロンプト送信済み

tmux session 一覧: tmux ls
切替: tmux switch-client -t <session_name>
```
...
- `gw_add` は tmux switch-client を含むため直接呼び出さない。worktree 作成・tmux session 作成・settings コピーを個別に実行する
