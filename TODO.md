...
- Grafana の週別 perm rate 計算を session 単位集計に修正
  - [ ] `perm rate 週別トレンド` が `permission_events` JOIN で `tool_use_total` を重複加算しない
  - [ ] `pr_metrics` と同様に permission 件数を session 単位で事前集計してから計算する
  - [ ] fixture DB で permission が複数あるセッションでも denominator が膨らまないことを確認する
...
- 不要になった PreToolUse hook 登録を整理
  - [ ] ADR-014 の設計どおり PermissionRequest 単体で permission 内訳が記録できることを確認する
  - [ ] `install` が不要な `pre-tool-use` を新規登録しない
  - [ ] 既存設定からの移行または互換方針を docs に記載する
